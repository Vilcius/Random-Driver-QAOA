...
def calc_expectation_random_qaoa_analytical_p1(angles: ndarray, graph: Graph, graph_random: Graph, edge_list: list[tuple[int, int]] = None) -> float:
    ...
    for u, v in edge_list:
        ...
        # Triangle terms
        if len(f) != 0:
            cos_prod_f_plus = math.prod([cos(graph_random.edges[u, m]['gamma'] + graph_random.edges[v, m]['gamma']) for m in f])
            cos_prod_f_minus = math.prod([cos(graph_random.edges[u, m]['gamma'] - graph_random.edges[v, m]['gamma']) for m in f])
            cuv += 1 / 4 * sin(2 * betas[u]) * sin(2 * betas[v]) * cos_prod_d * cos_prod_e * (cos_prod_f_plus - cos_prod_f_minus)
        ...
        if u in graph_random[v]:
            cuv += 1 / 2 * sin(graph_random.edges[u, v]['gamma']) * \
                (sin(2 * betas[u]) * cos(2 * betas[v]) * cos_prod_d + cos(2 * betas[u]) * sin(2 * betas[v]) * cos_prod_e)
        objective += cuv
